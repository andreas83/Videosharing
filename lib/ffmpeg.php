...
class FFmpeg {
    ...
    public function convertVideo($type="webm"){
        ...
        if($type=="webm")
            ...
            $pid=shell_exec($this->converter ." -i ".$this->filename." -codec:v libvpx -quality good -cpu-used 0 -b:v 600k -maxrate 600k -bufsize 1200k -qmin 10 -qmax 42 -vf scale=-1:480 -threads 0 -codec:a vorbis -b:a 128k ".$this->filename.".".$type);
        ...
        if($type=="mp4"){
           ...
           $preset=Config::get('basedir').'/lib/ffmpeg-presets/libx264-hq.ffpreset';
